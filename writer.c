...
int main() {
 ...
 if ( (buf_id = shm_open(shar_object, O_CREAT|O_RDWR, 0666)) == -1 ) {
  ...
  while (1) {
    ...
    ++buffer->current_pos;
    buffer->current_pos %= BUF_SIZE;
    buffer->store[buffer->current_pos] = rand() % 11; // число от 0 до 10
    ...
    printf("Writes value = %d to cell [%d]\n",
             buffer->store[buffer->current_pos], buffer->current_pos);
